...
export const PerformanceScreen: React.FC<PerformanceScreenProps> = ({ navigation }) => {
  ...
  const { examSlug, activeExam } = useExam();
  ...
  const fetchPerformance = useCallback(async () => {
    ...
    try {
      const [summary, revStats, plan, streak] = await Promise.all([
        ...
        revisionApi.getRevisionStats(examSlug).catch(() => null),
        studyPlanApi.getTodayStudyPlan(examSlug).catch(() => null),
  ...
  // Clear stale data when exam changes so different exams don't show each other's stats
  useEffect(() => {
    setData(null);
  ...
  const examName = activeExam?.name || 'Exam';
    ...
    <View style={styles.screenRoot}>
      ...
      <View style={[styles.headerContainer, { paddingTop: topInset }]}>
        ...
        <View style={styles.headerTitleWrap}>
          ...
          <View style={styles.headerExamBadge}>
            <Text style={styles.headerExamBadgeText}>{activeExam?.shortName || examName}</Text>
          </View>
        </View>
        <TouchableOpacity
          onPress={onRefresh}
      ...
      >
        ...
        {!hasData && (
          <View style={styles.onboardingCard}>
            ...
            <Text style={styles.onboardingTitle}>Build Your {examName} Profile</Text>
            <Text style={styles.onboardingDesc}>
              Attempt at least 5 questions across {examName} Practice Drills or Mock Tests. We will automatically detect your weak topics, calculate accuracy metrics, and provide custom drills!
            </Text>
            <TouchableOpacity
          ...
          <View style={styles.heroCard}>
            <View style={styles.heroTopRow}>
              ...
              <View style={styles.heroCircleBadge}>
                <Text style={styles.heroCircleIcon}>{activeExam.icon}</Text>
              </View>
            </View>
        ...
        <View style={styles.sectionCard}>
          ...
          >
            {[
              ...
              ...(data?.subjects || []).map((s) => ({
                label: s.subjectName.replace(/\s*\(Mains.*?\)/gi, '').replace(/\s*\(Paper.*?\)/gi, ''),
                slug: s.subjectSlug,
              })),
            ].map((p) => (
              <TouchableOpacity
