...
export const SplashScreen: React.FC<SplashScreenProps> = ({ navigation }) => {
  const { isLoaded, isFirstTime } = useExam();
  ...
  useEffect(() => {
    if (!isLoaded) return; // Wait for AsyncStorage to resolve

    const timer = setTimeout(() => {
      if (isFirstTime) {
        // New user → show onboarding then exam selection
        navigation.replace('Onboarding');
      } else {
        // Returning user with saved exam → go directly to subjects
        navigation.replace('Subjects');
      }
    }, 1200);

    return () => clearTimeout(timer);
  }, [navigation, isLoaded, isFirstTime]);
