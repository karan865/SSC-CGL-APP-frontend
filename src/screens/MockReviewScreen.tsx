...
export const MockReviewScreen: React.FC<MockReviewScreenProps> = ({ route, navigation }) => {
  ...
  const sectionNames = ['Reasoning', 'GA', 'Quant', 'English'];
      ...
      <View style={styles.sectionFilterRow}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.sectionPillsContainer}>
          ...
          {sectionNames.map((name, idx) => (
            <TouchableOpacity
              key={name}
              style={[styles.sectionPill, selectedSectionIndex === idx && styles.sectionPillActive]}
              onPress={() => setSelectedSectionIndex(idx)}
            >
              <Text style={[styles.sectionPillText, selectedSectionIndex === idx && styles.sectionPillTextActive]}>
                {name} (25)
              </Text>
            </TouchableOpacity>
          ))}
              ...
              <View key={q.questionId} style={styles.reviewCard}>
                ...
                <View style={styles.optionsList}>
                  {(['A', 'B', 'C', 'D'] as ('A' | 'B' | 'C' | 'D')[]).map((choice) => {
                    const optText = q[`option${choice}` as keyof MockReviewQuestion] as string;
                    ...
                    return (
                      ...
                      >
                        <View style={styles.reviewOptionHeader}>
                          ...
                          <Text style={styles.reviewOptionBody}>{optText}</Text>
                        </View>
                        {badgeLabel.length > 0 && (
                          <Text
                            style={[
                ...
                <View style={styles.solutionBox}>
                  <Text style={styles.solutionTitle}>💡 Step-by-Step Derivation:</Text>
